...
/** 取需求线标签：优先关联需求，其次项目 / 产品 / 迭代；都无则空串 */
...
/**
 * 首页收件箱洞察。
 * 在首页 InboxDeck（及未来其它首页组件）setup 中调用一次，拿到响应式的预测表与汇总。
 */
export function useInboxInsights() {
  ...
  /** 三类来源合并的归一化工作项（与首页 InboxDeck 的清单同源） */
  ...
  /** key = `${kind}-${id}`，与首页 InboxDeck 的卡片 key 同口径；仅含有风险的工作项 */
