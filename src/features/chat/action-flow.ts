...
function takeoverRules(extra = ''): string {
  // 知识库相关的「检索知识库」动作只在已配置时给出——未配置时 kb.search 工具
  // 不会下发给模型，提示它去检索只会得到「未知工具」的报错。
  ...
  return [
    ...
    `3. **我可以继续接手**：列出 3~5 个下一步动作，必须包含适用项：${handoffOptions}。`,
    '4. **需要你确认的事**：如果下一步会新建/修改/删除待办或执行 Git 等写操作，先问确认，不要直接做。',
    '涉及禅道任务/Bug时，如现有信息不足，先调用详情工具补齐' +
      (kbEnabled ? '；涉及内部流程或模块说明时，先检索知识库。' : '。'),
    extra,
  ]
    .filter(Boolean)
    .join('\n')
