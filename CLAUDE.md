...
- **环绕 HUD（方案 B）：** 顶部大总数 + 风险概况（逾期 / 临期 / 停滞计数）／三颗来源「卫星」（任务绿 / Bug 红 / 本地蓝，count-up 数字 + 该类风险构成迷你条）／右下位置读数（03/24 式）／**队列可视化**（见下方「队列可视化主题」）。卫星与可视化全部从 `deck` 派生，与卡堆天然同步；窄屏（≤1100px）卫星收拢为底部横排。
- **交互：** 滚轮 / 上下拖拽（45% 阻尼橡皮筋跟手）/ ↑↓·PageUp·Down / Home·End / 点侧卡 / 点可视化节点跳卡；拖拽位移超阈值吞掉尾迹点击；点标题直接看详情。

#### 队列可视化主题（`src/composables/useDeckTheme.ts` + `src/components/viz/DeckViz.vue` + `src/components/status/DeckThemeSwitch.vue`）

把「队列总数 / 进度」从一根平面圆环升级为**有体积的真 3D 结构**（圆环与卡堆同平面、被卡堆遮挡且无立体感——这是根因）。5 种结构全部节点分布在卡堆轮廓之外的 3D 空间，近端大且亮、远端小且暗（近大远小 = 真 3D）。状态栏天气左边提供切换器（`DeckThemeSwitch`，弹出面板任选），默认 **J 弧面副卡**；选择持久化到 `localStorage` / `hao123-deck-theme`（模块级单例 ref，状态栏与卡堆共享同一份状态）。

| 主题 | 结构 | 关键几何 |
|---|---|---|
| F 轨道光珠 | 光珠沿倾斜椭圆轨道环绕卡堆（含淡轨道环） | 半径 360 > 卡堆半宽，近端亮大 |
| G 螺旋天梯 | 节点绕卡堆盘旋上升成 DNA 螺旋 | 半径 360、纵向 ±260，均超出卡堆轮廓 |
| H 纵深回廊 | 比卡堆更大的发光门框向后上方没入雾中 | 门框宽 560+，近框形成「门洞」光晕 |
| I 全息晶柱 | 网格地板上每项一根发光晶柱（颜色=来源） | 半径 300 环阵，当前项最高最亮 |
| J 弧面副卡（默认） | 迷你副卡排成向两侧后退翘起的 3D 弧线（coverflow） | 相对当前项对称展开，近大远小 |

实现要点：`DeckViz` 接收 `items`（归一化 `deck`）+ `active` + `theme`，按主题 `v-if` 分支渲染对应节点，点击节点 `emit('jump', index)`；节点 `pointer-events:auto` 且 `stopPropagation` 避免触发卡堆拖拽，容器 `pointer-events:none`。
